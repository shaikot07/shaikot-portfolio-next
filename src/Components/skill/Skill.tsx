...
const Skill = () => {
    ...
    return (
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-8 justify-center items-center">
        {skills.map((skill) => (
          <Image key={skill.alt} src={skill.src} alt={skill.alt} width={84} height={84} className="mx-auto rounded-full" />
        ))}
      </div>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-6 justify-center items-center">
        {tools.map((tool) => (
          <Image key={tool.alt} src={tool.src} alt={tool.alt} width={64} height={64} className="mx-auto" />
        ))}
      </div>
    </div>
